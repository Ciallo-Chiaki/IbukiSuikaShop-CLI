...
const isPromise = (val) => {
  return (isObject$6(val) || isFunction$f(val)) && isFunction$f(val.then) && isFunction$f(val.catch);
};
...
function normalizeStyle(value) {
  if (isArray$t(value)) {
    ...
    return res;
  } else if (isString$a(value) || isObject$6(value)) {
    return value;
  }
}
...
const toDisplayString = (val) => {
  return isString$a(val) ? val : val == null ? "" : isArray$t(val) || isObject$6(val) && (val.toString === objectToString$3 || !isFunction$f(val.toString)) ? JSON.stringify(val, replacer, 2) : String(val);
};
const replacer = (_key, val) => {
  ...
  } else if (isObject$6(val) && !isArray$t(val) && !isPlainObject$8(val)) {
    return String(val);
  }
  return val;
};
...
class BaseFormatter {
  ...
  interpolate(message, values2, delimiters = defaultDelimiters) {
    if (!values2) {
      return [message];
    }
    ...
    return compile$1(tokens, values2);
  }
}
...
function compile$1(tokens, values2) {
  const compiled = [];
  let index2 = 0;
  const mode = Array.isArray(values2) ? "list" : isObject$5(values2) ? "named" : "unknown";
  if (mode === "unknown") {
    return compiled;
  }
  while (index2 < tokens.length) {
    ...
    switch (token.type) {
      case "text":
        ...
        break;
      case "list":
        compiled.push(values2[parseInt(token.value, 10)]);
        break;
      case "named":
        if (mode === "named") {
          compiled.push(values2[token.value]);
        } else {
          {
...
class I18n {
  ...
  f(message, values2, delimiters) {
    return this.formater.interpolate(message, values2, delimiters).join("");
  }
  t(key, locale, values2) {
    ...
    return this.formater.interpolate(message[key], values2).join("");
  }
}
...
function initVueI18n(locale, messages = {}, fallbackLocale, watcher) {
  ...
  let t2 = (key, values2) => {
    if (typeof getApp !== "function") {
      t2 = function(key2, values22) {
        return i18n.t(key2, values22);
      };
    } else {
    ...
    return t2(key, values2);
  };
  return {
    i18n,
    f(message, values2, delimiters) {
      return i18n.f(message, values2, delimiters);
    },
    t(key, values2) {
      return t2(key, values2);
    },
...
class MutableReactiveHandler extends BaseReactiveHandler {
  ...
  set(target, key, value, receiver) {
    ...
    const hadKey = isArray$t(target) && isIntegerKey(key) ? Number(key) < target.length : hasOwn$1(target, key);
...
function patchMPEvent(event, instance) {
  if (event.type && event.target) {
    ...
    if (isPlainObject$8(event.detail) && hasOwn$1(event.detail, "checked") && !hasOwn$1(event.detail, "value")) {
